...
function DragDropFiles() {
  ...
  function dropHandler(event) {
    event.preventDefault();
    setFiles((currentFiles) => [...currentFiles, ...event.dataTransfer.files]);
  }

  function changeFilesHandler(event) {
    setFiles((currentFiles) => [...currentFiles, ...event.target.files]);
  }
          ...
          {files.map((file, id) => (
            <li key={id}>
              <div className='files-row'>
                <svg
                  onClick={removeFileHandler.bind(null, file.name)}
